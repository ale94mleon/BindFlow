...
rule equil_setup_complex:
    input:
        mdp=expand(TemplatePath.complex.membrane.equi+"/{step}.mdp", step=[os.path.splitext(step)[0] for step in list_if_file(TemplatePath.complex.membrane.equi, ext='mdp')])
    params:
        ...
        steps = [os.path.splitext(step)[0] for step in list_if_file(TemplatePath.complex.membrane.equi, ext='mdp')],
    output:
        mdp=expand(run_path+"/complex/equil-mdsim/{step}/{step}.mdp", step=[os.path.splitext(step)[0] for step in list_if_file(TemplatePath.complex.membrane.equi, ext='mdp')])
    run:
        # Create MDP template
