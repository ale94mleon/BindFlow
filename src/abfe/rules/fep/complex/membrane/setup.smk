...
rule fep_setup_complex:
    input:
        ...
        mdp_vdw=expand(TemplatePath.complex.membrane.fep+"/vdw/{step}.mdp", step=[os.path.splitext(step)[0] for step in list_if_file(TemplatePath.complex.membrane.fep+"/vdw", ext='mdp')]),
        mdp_coul=expand(TemplatePath.complex.membrane.fep+"/coul/{step}.mdp", step=[os.path.splitext(step)[0] for step in list_if_file(TemplatePath.complex.membrane.fep+"/coul", ext='mdp')]),
        mdp_bonded=expand(TemplatePath.complex.membrane.fep+"/bonded/{step}.mdp", step=[os.path.splitext(step)[0] for step in list_if_file(TemplatePath.complex.membrane.fep+"/bonded", ext='mdp')])
    params:
    ...
    output:
        mdp_vdw=expand(run_path+"/complex/fep/simulation/vdw.{state}/{step}/{step}.mdp", state=range(len(config['lambdas']['complex']['vdw'])), step=[os.path.splitext(step)[0] for step in list_if_file(TemplatePath.complex.membrane.fep+"/vdw", ext='mdp')]),
    ...
    run:
        ...
        for itp_file in list_if_file(input.complex_top_dir, ext = 'ndx'):
            shutil.copy(os.path.join(input.complex_top_dir, itp_file), params.sim_dir+"/topology")
