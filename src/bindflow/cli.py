...
def abfe_run():
    ...
    parser.add_argument('-hmr', "--hmr_factor",
                        help="The Hydrogen Mass Repartition factor to use. 4 fs of integration time step will"
                        "be used no matter what hmf_factor is provided. "
    ...
    parser.add_argument('-lj', "--ligand_jobs",
                        help='Number of jobs in parallel for receptor workflow. '
                        'By defaults it will take number of ligands * replicas',
                        required=False, default=None, type=int)
    parser.add_argument('-jlj', "--jobs_per_ligand_job",
                        help='Number of jobs in parallel for each ligand workflow. By defaults it will take number of ligands * replicas',
                        required=False, default=10000, type=int)
    parser.add_argument('-r', "--replicas",
                        help='Number of replicates',
                        required=False, default=3, type=int)
    parser.add_argument('-gc', '--global_config',
                        help="This is the configuration YAML file for your simulation. Here you define the cluster characteristics "
                        "and other optional parameters of the simulation",
...
def abfe_dag():
    ...
    parser.add_argument('-p', "--path",
                        help='Where should the dag be executed (where the Snakefile is located)',
                        type=str, default='.')
...
def abfe_archive():
    from bindflow._version import __version__
    from bindflow.utils.tools import archive
    parser = argparse.ArgumentParser()
    parser.add_argument(
        dest='root_path',
...
def abfe_unarchive():
    from bindflow._version import __version__
    from bindflow.utils.tools import unarchive
    parser = argparse.ArgumentParser()
    parser.add_argument(
        dest='archive_file',
