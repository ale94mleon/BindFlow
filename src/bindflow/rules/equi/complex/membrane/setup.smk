rule equil_setup_complex:
    input:
        mdp=expand(TemplatePath.complex.membrane.equi+"/{step}.mdp", step=[Path(step).stem for step in tools.list_if_file(TemplatePath.complex.membrane.equi, ext='mdp')])
    params:
        ...
        steps=[Path(step).stem for step in tools.list_if_file(TemplatePath.complex.membrane.equi, ext='mdp')],
        ligand_names=config['ligand_names'],
        replicas=range(1,1 + config['replicas']),
    output:
        mdp=expand(out_approach_path+"/{ligand_name}/{replica}/complex/equil-mdsim/{step}/{step}.mdp", step=[Path(step).stem for step in tools.list_if_file(TemplatePath.complex.membrane.equi, ext='mdp')], ligand_name=config['ligand_names'], replica=list(map(str, range(1,1 + config['replicas']))))
    run:
