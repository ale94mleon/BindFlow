rule equil_setup_ligand:
    input:
        mdp = expand(str(TemplatePath.ligand.equi/"{step}.mdp"), step = [step.stem for step in tools.list_if_file(TemplatePath.ligand.equi, ext='.mdp')])
    params:
        ...
        steps = [step.stem for step in tools.list_if_file(TemplatePath.ligand.equi, ext='.mdp')],
        ligand_names = config['ligand_names'],
        replicas = range(1,1 + config['replicas']),
    output:
        mdp = expand(out_approach_path+"/{ligand_name}/{replica}/ligand/equil-mdsim/{step}/{step}.mdp", step = [step.stem for step in tools.list_if_file(TemplatePath.ligand.equi, ext='.mdp')], ligand_name = config['ligand_names'], replica = list(map(str, range(1,1 + config['replicas']))))
    run:
