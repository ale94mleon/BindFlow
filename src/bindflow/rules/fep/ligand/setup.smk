rule fep_setup_ligand:
    input:
        mdp_vdw=expand(TemplatePath.ligand.fep+"/vdw/{step}.mdp", step=[Path(step).stem for step in tools.list_if_file(TemplatePath.ligand.fep + "/vdw", ext='mdp')]),
        mdp_coul=expand(TemplatePath.ligand.fep+"/coul/{step}.mdp", step=[Path(step).stem for step in tools.list_if_file(TemplatePath.ligand.fep + "/coul", ext='mdp')])
    params:
